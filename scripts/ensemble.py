...
parser.add_argument("--merge", type=str, required=True, choices=list(priors_ensemble.STRATEGIES))
parser.add_argument("--alpha", type=float, default=None, help="only used if merge is `weighted` or `ranked`.")
parser.add_argument('--allow_partial_merge', default=False, type=str2bool, help="ensemble valid CAMs if some are missing or corrupted.")
...
def _work(process_id, dataset: Subset, args, experiments, cam_dirs, weights, out_dir):
  ...
  with torch.no_grad():
    for image_id, image_path, mask_path in subset:
      ...
      if not args.allow_partial_merge and len(partial_cams) < len(cam_dirs):
        continue  # do not allow partial CAM ensembles.

      try:
        cam = priors_ensemble.merge(partial_cams, targets, args.merge, weights=weights, alpha=args.alpha)
