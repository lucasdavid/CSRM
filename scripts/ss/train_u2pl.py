...
import datasets
import wandb
from singlestage import CSRM, u2pl, reco
...
def train_u2pl(args, wb_run, model_path):
  ...
  criterions = (
    ...
    u2pl.compute_unsupervised_loss,  # torch.nn.CrossEntropyLoss(ignore_index=255, label_smoothing=args.label_smoothing).to(DEVICE),
  )
      ...
      for step, batch in enumerate(tqdm_custom(train_l_loader, desc=f"Epoch {epoch}")):
        ...
        if (step + 1) % args.accumulate_steps == 0:
          ...
          if not args.warmup_epochs or epoch >= args.warmup_epochs:
            with torch.no_grad():
                ema_decay = min(1 - 1 / (optimizer.global_step - len(loader_l) * cfg["trainer"].get("sup_only_epoch", 1) + 1), ema_decay_origin,)
...
def train_step(
  ...
  if not w_s2c:
    ...
    loss_s2c = (
      u2pl.compute_unsupervised_loss(
          pred_u_large,
          label_u_aug.clone(),
          drop_percent,
